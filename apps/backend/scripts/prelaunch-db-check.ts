...
async function main(): Promise<void> {
  const se = await db.query<{ status: string; c: string }>(
  ...
  const st = await db.query<{ c: string }>(`SELECT count(*)::text AS c FROM spot_trades`);
  ...
  const sle = await db.query<{ c: string }>(`SELECT count(*)::text AS c FROM settlement_ledger_entries`);
