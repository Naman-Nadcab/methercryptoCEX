...
export function P2PHeader() {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-gray-200/90 bg-card/95 backdrop-blur dark:border-border/90 dark:bg-card/95">
      <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3 px-4 py-2.5">
        <div className="flex items-center gap-2">
          <Link href={P2P_HREF} className="text-sm font-semibold text-foreground">
            P2P Trading
          </Link>
        </div>
        ...
        <div className="flex items-center gap-2">
          {authed ? (
            <Link
              href={ROUTES.home}
              className="inline-flex items-center gap-1 rounded-lg border border-border px-2 py-1 text-xs text-muted-foreground hover:bg-muted dark:border-border dark:text-foreground/80 dark:hover:bg-accent"
            >
