...
export default function EmailTemplatesPage() {
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/api/v1/admin/notifications/email-templates`, {
  ...
  const handleSubmit = async () => {
    if (!accessToken) return;
    setError('');
    if (!form.slug.trim() || !form.name.trim() || !form.subject.trim() || !form.body_html.trim()) {
      setError('Slug, name, subject and body (HTML) are required.');
      return;
    }
    setSaving(true);
    try {
      ...
      if (editing) {
        const res = await fetch(`${apiUrl}/api/v1/admin/notifications/email-templates/${editing.id}`, {
          method: 'PATCH',
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const apiUrl = getApiBaseUrl();
      const res = await fetch(`${apiUrl}/api/v1/admin/notifications/email-templates/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      const result = await res.json();
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Manage transactional and marketing email templates (e.g. welcome, withdrawal, 2FA).</p>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-gray-500 dark:text-gray-400">No email templates. Add one for transactional or marketing emails.</div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/80">
                  ...
                  <th className="text-left px-6 py-4 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Subject</th>
              ...
              <tbody>
                {list.map((t) => (
                  <tr key={t.id} className="border-b border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/20">
                    ...
                    <td className="px-6 py-4 text-gray-500 dark:text-gray-400 truncate max-w-[200px]">{t.subject}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => !saving && setShowModal(false)}>
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{editing ? 'Edit Email Template' : 'New Email Template'}</h2>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={form.slug} onChange={(e) => setForm((f) => ({ ...f, slug: e.target.value }))} className="w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white" placeholder="e.g. welcome_email" disabled={!!editing} />
                </div>
                <div>
              ...
              <div>
                ...
                <input type="text" value={form.subject} onChange={(e) => setForm((f) => ({ ...f, subject: e.target.value }))} className="w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white" placeholder="Email subject" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Body (HTML) *</label>
                <textarea value={form.body_html} onChange={(e) => setForm((f) => ({ ...f, body_html: e.target.value }))} rows={6} className="w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white font-mono text-sm" placeholder="HTML content. Use {{variable}} for placeholders." />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Body (plain text, optional)</label>
                <textarea value={form.body_text} onChange={(e) => setForm((f) => ({ ...f, body_text: e.target.value }))} rows={3} className="w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white" />
              </div>
