...
export default function ApprovedKYCPage() {
  ...
  const fetchApproved = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_URL}/api/v1/admin/kyc?page=${page}&limit=${limit}&status=approved`, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      const data = await res.json();
      if (!res.ok) {
        setError(data?.error?.message ?? 'Failed to load approved KYC');
        return;
      }
  ...
  return (
    <div className="space-y-6">
      <SectionHeader title="Approved KYC" subtitle="Approved KYC applications (from backend)." />
      {error && (
      ...
      <Panel>
        {loading && !applications.length ? (
          ...
          <p className="text-gray-500 text-center py-8">No approved KYC yet</p>
        ) : (
          <DataTableContainer>
            <DataTableHead>
              <DataTableRow>
