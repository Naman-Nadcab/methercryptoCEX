...
export default function CompletedWithdrawalsPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/api/v1/admin/withdrawals?status=completed&limit=100`, {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Completed Withdrawals</h1>
        <p className="text-gray-400 text-sm mt-1">Successfully processed withdrawal history.</p>
      ...
      <div className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
        {withdrawals.length === 0 ? (
          <div className="p-8 text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400">No completed withdrawals yet</p>
          </div>
        ) : (
              ...
              <tbody>
                {withdrawals.map((w) => (
                  <tr key={w.id} className="border-b border-gray-200 dark:border-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-700/20">
                    ...
                    <td className="px-6 py-4 text-gray-400 truncate max-w-[180px]">{w.to_address}</td>
                    <td className="px-6 py-4">
                      {w.tx_hash ? (
                        <a href={`https://etherscan.io/tx/${w.tx_hash}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline truncate max-w-[120px] block">
