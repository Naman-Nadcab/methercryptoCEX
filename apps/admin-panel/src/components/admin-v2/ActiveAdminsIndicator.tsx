...
function ActiveAdminsIndicatorInner() {
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-2 rounded-xl border border-admin-border bg-admin-card">
      <Users className="w-3.5 h-3.5 text-admin-muted" />
      ...
      <div className="flex flex-col gap-0.5 min-w-0">
        {display.map((admin) => (
          <div key={admin.name} className="flex items-center gap-1.5 text-[10px] leading-tight">
            ...
            <span className="text-admin-text font-medium truncate">{admin.name}</span>
            <span className="text-admin-muted truncate">({admin.page})</span>
          </div>
        ))}
      </div>
    </div>
