...
function DashboardV2Inner() {
  ...
  // Stable refs for store actions to prevent useEffect infinite loops
  ...
  const predictiveLinkerRef = useRef(predictiveLinker);
  predictiveLinkerRef.current = predictiveLinker;
  const activeIncidentIdRef = useRef(activeIncidentId);
  activeIncidentIdRef.current = activeIncidentId;

  // Loop guard: skip if metrics haven't materially changed
  const lastMetricsKeyRef = useRef('');

  // Run alert engine + incident detector + predictive ops (debounced)
  useEffect(() => {
    if (!health && !control) return;

    const timeout = setTimeout(() => {
      // Fingerprint current metrics to detect real changes
      ...
      if (ADMIN_FEATURE_FLAGS.ADMIN_AI_OPS) {
        ...
        if (predictions.length > 0) {
          ...
          addPredictiveAlertsRef.current(predictiveAlertsList);
          predictiveLinkerRef.current.trackPredictions(predictions);
        }

        if (alerts.length > 0) {
          predictiveLinkerRef.current.checkRealAlerts(alerts, activeIncidentIdRef.current);
        }
      }
    }, 300);
