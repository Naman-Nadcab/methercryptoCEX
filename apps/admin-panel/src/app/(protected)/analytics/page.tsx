  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-lg font-semibold text-admin-text">Exchange Analytics</h1>
          <p className="text-xs text-admin-muted mt-0.5">Revenue, volume, liquidity, user growth, and business intelligence</p>
        </div>
              ...
              {(volume?.volume_by_market ?? []).length === 0 ? <EmptyChart message="No volume data yet" /> : (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={(volume?.volume_by_market ?? []).slice(0, 8)} margin={{ left: 10, right: 10, top: 5, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#F1F5F9" />
                    <XAxis dataKey="market" tick={{ fontSize: 10, fill: '#94A3B8' }} />
                    <YAxis tick={{ fontSize: 10, fill: '#94A3B8' }} tickFormatter={(v) => `$${(v / 1e6).toFixed(0)}M`} />
        ...
        <div className="rounded-xl border border-admin-border bg-admin-card">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[650px] text-left text-xs">
              <thead>
                <tr className="border-b border-admin-border bg-white/[0.02]/80">
              ...
              <tbody>
                {markets.length === 0 ? (
                  ...
                  <tr key={row.market} className="border-b border-admin-border/50 last:border-0 hover:bg-white/[0.02]/50">
                ...
                <table className="w-full text-left text-xs">
                  ...
                  <tbody>
                    {(whaleAlerts?.top_whale_users ?? []).map((u, i) => (
                      <tr key={i} className="border-b border-admin-border/50 last:border-0 hover:bg-white/[0.02]/50">
...
const tooltipStyle: React.CSSProperties = { fontSize: 11, borderRadius: 8, border: '1px solid #E2E8F0', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' };
...
function VolatilityBadge({ value }: { value: number }) {
  ...
  return (
    <span className={cn('inline-flex items-center gap-0.5 text-xs tabular-nums font-medium',
      isHigh ? 'text-red-600' : isMed ? 'text-amber-600' : 'text-admin-muted')}>
