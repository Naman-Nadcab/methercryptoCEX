...
import { Coins, Plus, Power, PowerOff, Loader2, AlertTriangle, Pencil } from 'lucide-react';
...
type EditForm = { apyPct: string; lockPeriodDays: string; minStake: string };
...
export default function StakingPage() {
  ...
  const editMut = useMutation({
    mutationFn: ({ id, body }: { id: string; body: Record<string, unknown> }) =>
      adminFetch(`/staking/products/${id}`, { method: 'PATCH', body, token }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin', 'staking-products'] });
      setEditTarget(null);
  ...
  function openEdit(p: StakingProduct) {
    setEditTarget(p);
    setEditForm({
      apyPct: String(p.apy_pct),
      lockPeriodDays: String(p.lock_period_days),
      minStake: String(p.min_stake),
    });
  }

  function submitEdit() {
    if (!editTarget) return;
    const apy = Number(editForm.apyPct);
    const lock = Number(editForm.lockPeriodDays);
    const min = Number(editForm.minStake);
  ...
  return (
    <AdminPageFrame
      title="Staking & Earn"
      description="Manage staking products and earn pools."
      quickActions={
    ...
    >
                    ...
                    <tr key={p.id} className="border-b border-admin-border/60 hover:bg-white/5">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <ProtectedAction permission="settings:edit" fallback="disabled">
                            <Button type="button" size="sm" variant="ghost" className="whitespace-nowrap"
      ...
      <Modal open={!!editTarget} onClose={() => setEditTarget(null)} title={`Edit: ${editTarget?.name ?? ''}`} size="md">
        <div className="space-y-4">
          <Input label="APY (%)" type="number" step="0.01" value={editForm.apyPct} onChange={(e) => setEditForm((f) => ({ ...f, apyPct: e.target.value }))} placeholder="5.25" />
          <div className="grid grid-cols-2 gap-4">
            <Input label="Lock period (days)" type="number" min={0} value={editForm.lockPeriodDays} onChange={(e) => setEditForm((f) => ({ ...f, lockPeriodDays: e.target.value }))} placeholder="30" />
            <Input label="Minimum stake ($)" type="number" min={0} value={editForm.minStake} onChange={(e) => setEditForm((f) => ({ ...f, minStake: e.target.value }))} placeholder="100" />
          </div>
        </div>
        <ModalFooter className="mt-4 border-0 px-0 pb-0 pt-4">
          ...
          <Button type="button" onClick={submitEdit} loading={editMut.isPending}>Save changes</Button>
        </ModalFooter>
      </Modal>
